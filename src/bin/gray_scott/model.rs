...
impl Resources {
    fn new(device: &Device, config: &SurfaceConfiguration) -> Self {
        ...
        let cell_bind_group_layout = BindGroupLayoutBuilder::new()
            .add_storage_buffer(wgpu::BufferSize::new((n_cells * 4 * 2) as _), true)
            .add_storage_buffer(wgpu::BufferSize::new((n_cells * 4 * 2) as _), false)
            .build(device, None);
        ...
        let init_cells = Self::create_init_cells(n_rows);
    ...
    fn create_init_cells(n_rows: usize) -> Vec<f32> {
        let mut rng = rand::rngs::StdRng::seed_from_u64(222);
        let unif = Uniform::new::<f32, f32>(0.0, 0.1);
        let d = 20.0;
        let cells = Vec::from_iter((0..(n_rows * n_rows)).flat_map(|i| {
            let x = (i % n_rows) as f32;
            let y = (i / n_rows) as f32;
            let half = n_rows as f32 / 2.0;
            let mut u = 1.0;
            let mut v = 0.0;
            if ((x - half).abs() < d) && ((y - half).abs() < d) {
                u += 0.5;
                v += 0.25;
            }
            u += unif.sample(&mut rng);
            v += unif.sample(&mut rng);
            [u, v]
        }));
        return cells;
    }
