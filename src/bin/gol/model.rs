use wgpu::{Device, Queue, Surface, SurfaceConfiguration, BufferUsages, Buffer};
use winit::window::Window;
use rand::{distributions::{Distribution, Uniform}, SeedableRng};
use bytemuck;

use crate::util::{BindGroupBuilder, BindGroupLayoutBuilder, create_compute_pipeline, create_buffer, create_render_pipeline};
...
impl Resources {
    fn new(device: &Device, config: &SurfaceConfiguration) -> Self {
        ...
        let compute_pipeline = create_compute_pipeline(device, &[&cell_bind_group_layout], &compute_shader);

        let mut rng = rand::rngs::StdRng::seed_from_u64(111);
        let unif = Uniform::new::<f32, f32>(-0.5, 0.5);
        let init_cells: Vec<f32> = Vec::from_iter((0..n_cells).map(|_| {
            unif.sample(&mut rng).ceil()
        }));
        ...
        let cell_buffers: Vec<Buffer> = Vec::from_iter((0..2).map(|i| {
            create_buffer(device, bytemuck::cast_slice(&init_cells), usage, Some(&format!("cell buffer {}", i)))
        }));
        ...
        let vertices_buffer = create_buffer(device, bytemuck::bytes_of(&vertices), BufferUsages::VERTEX | BufferUsages::COPY_DST, None);
        ...
        let render_pipeline = create_render_pipeline(device, config, vertex_buffer_layouts, &draw_shader);
    ...
    fn render_pass(&self, encoder: &mut wgpu::CommandEncoder, color_attachments: &[wgpu::RenderPassColorAttachment], frame_num: usize) {
...
impl Model {
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        self.resources.render_pass(&mut encoder, &color_attachments, self.frame_num);
